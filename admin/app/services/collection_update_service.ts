...
export class CollectionUpdateService {
  async checkForUpdates(): Promise<ContentUpdateCheckResult> {
    const roachnetAPIURL = env.get('ROACHNET_API_URL') || ROACHNET_API_DEFAULT_BASE_URL
    if (!roachnetAPIURL) {
      return {
        updates: [],
        checked_at: new Date().toISOString(),
        error: 'RoachNet API is not configured. Set the ROACHNET_API_URL environment variable.',
      }
    }
    ...
    try {
      const response = await axios.post<ResourceUpdateInfo[]>(`${roachnetAPIURL}/api/v1/resources/check-updates`, requestBody, {
        timeout: 15000,
      })
      ...
      if (axios.isAxiosError(error) && error.response) {
        logger.error(
          `[CollectionUpdateService] RoachNet API returned ${error.response.status}: ${JSON.stringify(error.response.data)}`
        )
        return {
          updates: [],
          checked_at: new Date().toISOString(),
          error: `RoachNet API returned status ${error.response.status}`,
        }
      }
      const message =
        error instanceof Error ? error.message : 'Unknown error contacting RoachNet API'
      logger.error(`[CollectionUpdateService] Failed to check for updates: ${message}`)
      return {
        updates: [],
        checked_at: new Date().toISOString(),
        error: `Failed to contact RoachNet API: ${message}`,
      }
    }
  }
