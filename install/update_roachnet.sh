...
ensure_docker_compose_file_exists() {
  if [ ! -f "/opt/roachnet/compose.yml" ]; then
    echo -e "${RED}#${RESET} compose.yml file not found. Please ensure it exists at /opt/roachnet/compose.yml."
    exit 1
  fi
}
...
force_recreate() {
  ...
  if ! docker compose -p roachnet -f /opt/roachnet/compose.yml pull; then
  ...
  if ! docker compose -p roachnet -f /opt/roachnet/compose.yml up -d --force-recreate; then
