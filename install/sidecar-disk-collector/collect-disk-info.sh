...
# No special capabilities required. Writes JSON to /storage/roachnet-disk-info.json, which is read by the admin container.
...
if [[ ! -f /storage/roachnet-disk-info.json ]]; then
    echo '{"diskLayout":{"blockdevices":[]},"fsSize":[]}' > /storage/roachnet-disk-info.json
...
while true; do
    ...
    cat > /storage/roachnet-disk-info.json.tmp << EOF
{
...
EOF
    ...
    if mv /storage/roachnet-disk-info.json.tmp /storage/roachnet-disk-info.json; then
        log "Disk info updated successfully."
    else
        log "ERROR: Failed to move temp file to /storage/roachnet-disk-info.json"
    fi
