...
create_roachnet_directory(){
  # Ensure the main installation directory exists
  if [[ ! -d "$ROACHNET_DIR" ]]; then
    echo -e "${YELLOW}#${RESET} Creating directory for RoachNet at $ROACHNET_DIR...\\n"
    sudo mkdir -p "$ROACHNET_DIR"
    sudo chown "$(whoami):$(whoami)" "$ROACHNET_DIR"

    echo -e "${GREEN}#${RESET} Directory created successfully.\\n"
  else
    echo -e "${GREEN}#${RESET} Directory $ROACHNET_DIR already exists.\\n"
  fi

  # Also ensure the directory has a /storage/logs/ subdirectory
  sudo mkdir -p "${ROACHNET_DIR}/storage/logs"

  # Create a admin.log file in the logs directory
  sudo touch "${ROACHNET_DIR}/storage/logs/admin.log"
}

download_management_compose_file() {
  local compose_file_path="${ROACHNET_DIR}/compose.yml"
...
download_helper_scripts() {
  local start_script_path="${ROACHNET_DIR}/start_roachnet.sh"
  local stop_script_path="${ROACHNET_DIR}/stop_roachnet.sh"
  local update_script_path="${ROACHNET_DIR}/update_roachnet.sh"
...
start_management_containers() {
  ...
  if ! sudo docker compose -p roachnet -f "${ROACHNET_DIR}/compose.yml" up -d; then
...
success_message() {
  ...
  echo -e "${GREEN}#${RESET} Installation files are located at /opt/roachnet\\n\n"
  echo -e "${GREEN}#${RESET} RoachNet Command Center should automatically start whenever your device reboots. However, if you need to start it manually, you can always do so by running: ${WHITE_R}${ROACHNET_DIR}/start_roachnet.sh${RESET}\\n"
